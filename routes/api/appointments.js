...
// @route           GET api/appointments/getِAllAppointments/:id
// @description:    get all appointments 
//@access           public
router.get('/getAllAppointments', auth , (req,res) => {
    Appointment.find()
        .sort({ date: -1 })
...
router.get('/appointmentIsExist/:id', auth , (req,res) => {

    Appointment.findById(req.params.id).then(appointment=>{
   if(appointment){
    res.status(200).json({msg: 'appointmentExists',flag:true});
   }
   else
   {
    res.status(200  ).json({msg: 'appointmentNotExists',flag:false});
   }
...
router.delete('/delete/:id', auth, (req,res) => {
    Appointment.findById(req.params.id)
        ...
        .then(() => res.json({appointment:appointment,success: true})))
        .catch(err => res.status(404).json({success: false}));
 });
 // @route           DELETE api/appointments/deleteUserAppointments/:userType/:id
// @description:    delete a appointmet
// @access           private
router.post('/deleteUserAppointments/:id', auth, (req,res) => {
    ...
    if (userType === 'patient'){
        ...
        .then(appointments => {
            appointments.map(appointment=>{
               appointment.remove()
               .then(() => res.json({appointments:appointments,success: true}))
               .catch(err => res.status(404).json({success: false}));
            })})
        .catch(err => res.status(404).json({success: false}));
    }
...
router.post('/makeReport/:id',auth,(req,res)=> {    
    Appointment.findOneAndUpdate({_id : req.params.id}, {report: req.body.report }, {
    new: true
    }).then(appointment => res.json(appointment))
    })
...
router.post('/updateAppointment/:id', auth, (req,res) => {
    ...
    { new: true })
    .then(appointment => res.json({appointment:appointment,updatedBy:req.user.id}
        ))
})
